...
const Footer = () => {
  ...
  return (
    <footer className="glass-card text-gray-300">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
          ...
          {linkSections.map((section) => (
            <div key={section.title}>
              <h4 className="text-gray-100 font-semibold mb-4 tracking-wider uppercase">{section.title}</h4>
              <ul className="space-y-3">
                {section.links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-gray-400 hover:text-gray-100 hover:underline transition">
                      {link}
                    </a>
                  </li>
                ))}
        ...
        <div className="border-t border-white/10 mt-12 pt-8 flex flex-col sm:flex-row justify-between items-center">
          ...
          <div className="flex items-center space-x-5">
            {socialLinks.map((social, index) => (
              <a key={index} href={social.href} className="text-gray-400 hover:text-gray-100 transition">
